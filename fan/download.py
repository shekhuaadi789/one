...
from fan.filesystem import is_file
...
def conditional_download(download_directory_path : str, urls : List[str]) -> None:
	...
	for url in urls:
		...
		if initial < total:
			with tqdm(total = total, initial = initial, desc = wording.get('downloading'), unit = 'B', unit_scale = True, unit_divisor = 1024, ascii = ' =', disable = fan.globals.log_level in [ 'warn', 'error' ]) as progress:
