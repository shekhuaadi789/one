...
from fan.core import conditional_append_reference_faces
from fan.face_store import clear_static_faces, get_reference_faces, clear_reference_faces
from fan.typing import Frame, Face, FaceSet
from fan.vision import get_video_frame, count_video_frame_total, normalize_frame_color, resize_frame_dimension, read_static_image, read_static_images
from fan.face_analyser import get_average_face, clear_face_analyser
from fan.content_analyser import analyse_frame
from fan.processors.frame.core import load_frame_processor_module
...
def render() -> None:
	...
	if is_image(fan.globals.target_path):
		target_frame = read_static_image(fan.globals.target_path)
	...
	if is_video(fan.globals.target_path):
		temp_frame = get_video_frame(fan.globals.target_path, fan.globals.reference_frame_number)
		preview_frame = process_preview_frame(source_face, reference_faces, temp_frame)
		preview_image_args['value'] = normalize_frame_color(preview_frame)
		preview_image_args['visible'] = True
		preview_frame_slider_args['value'] = fan.globals.reference_frame_number
		preview_frame_slider_args['maximum'] = count_video_frame_total(fan.globals.target_path)
...
def update_preview_image(frame_number : int = 0) -> gradio.Image:
	...
	source_frames = read_static_images(fan.globals.source_paths)
	source_face = get_average_face(source_frames)
	reference_face = get_reference_faces() if 'reference' in fan.globals.face_selector_mode else None
	if is_image(fan.globals.target_path):
		target_frame = read_static_image(fan.globals.target_path)
	...
	if is_video(fan.globals.target_path):
		temp_frame = get_video_frame(fan.globals.target_path, frame_number)
...
def update_preview_frame_slider() -> gradio.Slider:
	if is_video(fan.globals.target_path):
		video_frame_total = count_video_frame_total(fan.globals.target_path)
