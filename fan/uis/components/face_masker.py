...
from fan.typing import FaceMaskType, FaceMaskRegion
...
def render() -> None:
	...
	has_box_mask = 'box' in fan.globals.face_mask_types
	has_region_mask = 'region' in fan.globals.face_mask_types
	FACE_MASK_TYPES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		label = wording.get('face_mask_types_checkbox_group_label'),
		choices = fan.choices.face_mask_types,
		value = fan.globals.face_mask_types
	)
	with gradio.Group(visible = has_box_mask) as FACE_MASK_BOX_GROUP:
		FACE_MASK_BLUR_SLIDER = gradio.Slider(
			...
			step = fan.choices.face_mask_blur_range[1] - fan.choices.face_mask_blur_range[0],
			minimum = fan.choices.face_mask_blur_range[0],
			maximum = fan.choices.face_mask_blur_range[-1],
			value = fan.globals.face_mask_blur
		)
		with gradio.Row():
			FACE_MASK_PADDING_TOP_SLIDER = gradio.Slider(
				label = wording.get('face_mask_padding_top_slider_label'),
				step = fan.choices.face_mask_padding_range[1] - fan.choices.face_mask_padding_range[0],
				minimum = fan.choices.face_mask_padding_range[0],
				maximum = fan.choices.face_mask_padding_range[-1],
				value = fan.globals.face_mask_padding[0]
			)
			FACE_MASK_PADDING_RIGHT_SLIDER = gradio.Slider(
				...
				minimum = fan.choices.face_mask_padding_range[0],
				maximum = fan.choices.face_mask_padding_range[-1],
				value = fan.globals.face_mask_padding[1]
			)
		with gradio.Row():
			FACE_MASK_PADDING_BOTTOM_SLIDER = gradio.Slider(
				...
				minimum = fan.choices.face_mask_padding_range[0],
				maximum = fan.choices.face_mask_padding_range[-1],
				value = fan.globals.face_mask_padding[2]
			)
			FACE_MASK_PADDING_LEFT_SLIDER = gradio.Slider(
				...
				minimum = fan.choices.face_mask_padding_range[0],
				maximum = fan.choices.face_mask_padding_range[-1],
				value = fan.globals.face_mask_padding[3]
			)
	with gradio.Row():
		FACE_MASK_REGION_CHECKBOX_GROUP = gradio.CheckboxGroup(
			label = wording.get('face_mask_region_checkbox_group_label'),
			choices = fan.choices.face_mask_regions,
			value = fan.globals.face_mask_regions,
...
def update_face_mask_type(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.Group, gradio.CheckboxGroup]:
	if not face_mask_types:
		face_mask_types = fan.choices.face_mask_types
	fan.globals.face_mask_types = face_mask_types
...
def update_face_mask_blur(face_mask_blur : float) -> None:
	fan.globals.face_mask_blur = face_mask_blur
...
def update_face_mask_padding(face_mask_padding_top : int, face_mask_padding_right : int, face_mask_padding_bottom : int, face_mask_padding_left : int) -> None:
	fan.globals.face_mask_padding = (face_mask_padding_top, face_mask_padding_right, face_mask_padding_bottom, face_mask_padding_left)


def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	if not face_mask_regions:
		face_mask_regions = fan.choices.face_mask_regions
	fan.globals.face_mask_regions = face_mask_regions
