...
from fan.vision import get_video_frame, read_static_image, normalize_frame_color
from fan.face_analyser import get_many_faces
from fan.typing import Frame, FaceSelectorMode
...
def render() -> None:
	...
	reference_face_gallery_args: Dict[str, Any] =\
	{
		...
		'columns': 8,
		'allow_preview': False,
		'visible': 'reference' in fan.globals.face_selector_mode
	}
	if is_image(fan.globals.target_path):
		reference_frame = read_static_image(fan.globals.target_path)
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = fan.choices.face_selector_modes,
		value = fan.globals.face_selector_mode
	)
	REFERENCE_FACE_POSITION_GALLERY = gradio.Gallery(**reference_face_gallery_args)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = wording.get('reference_face_distance_slider_label'),
		value = fan.globals.reference_face_distance,
		step = fan.choices.reference_face_distance_range[1] - fan.choices.reference_face_distance_range[0],
		minimum = fan.choices.reference_face_distance_range[0],
		maximum = fan.choices.reference_face_distance_range[-1],
		visible = 'reference' in fan.globals.face_selector_mode
	)
...
def update_face_selector_mode(face_selector_mode : FaceSelectorMode) -> Tuple[gradio.Gallery, gradio.Slider]:
	if face_selector_mode == 'reference':
		fan.globals.face_selector_mode = face_selector_mode
...
def update_reference_face_position(reference_face_position : int = 0) -> None:
	fan.globals.reference_face_position = reference_face_position


def update_reference_face_distance(reference_face_distance : float) -> None:
	fan.globals.reference_face_distance = reference_face_distance


def update_reference_frame_number(reference_frame_number : int) -> None:
	fan.globals.reference_frame_number = reference_frame_number
