...
import fan.globals
from fan import metadata, logger, wording
from fan.uis.typing import Component, ComponentName
...
def load_ui_layout_module(ui_layout : str) -> Any:
	try:
		ui_layout_module = importlib.import_module('fan.uis.layouts.' + ui_layout)
...
def launch() -> None:
	with gradio.Blocks(theme = get_theme(), css = get_css(), title = metadata.get('name') + ' ' + metadata.get('version')) as ui:
		for ui_layout in fan.globals.ui_layouts:
			ui_layout_module = load_ui_layout_module(ui_layout)
